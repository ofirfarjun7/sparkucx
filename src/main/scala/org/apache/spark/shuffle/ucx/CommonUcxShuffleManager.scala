...
abstract class CommonUcxShuffleManager(val conf: SparkConf, isDriver: Boolean) extends SortShuffleManager(conf) {
  ...
  var ucxTransport: UcxShuffleTransport = _
  ...
  private val setupThread = new Thread() {
    setDaemon(true)
    setName("UcxTransportSetupThread")
   ...
   */
  def startUcxTransport(): Unit = if (ucxTransport == null) synchronized {
    if (ucxTransport == null) {
      val blockManager = SparkEnv.get.blockManager.blockManagerId
      ucxTransport = new UcxShuffleTransport(ucxShuffleConf, blockManager.executorId.toLong)
